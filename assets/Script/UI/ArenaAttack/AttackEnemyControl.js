...
cc.Class({
    ...
    init() {
        ...
        if (this.LoadControl) {
            ...
            //把所有的動畫裝載
            this.SkillAltasModel = this.LoadControl.getSKillAtlas('enemy');
            this.NormalAltas = this.LoadControl.getNormalAtlas('enemy');
            //建立技能動畫
    ...
    addAnimation(model) {
        /*
        {
            ...
            "TimeSecond": 1,
            "EffectType": "self",
            "LoadEffect": "AutoAtlas/Skills/enemy/enemy_sword",
            "atlas":{}
        }
        ,
        {
    ...
    useSkillAnimation(SkillID) {
        ...
        console.log(Skill);
        if (Skill) var ClipName = Skill.EffectEnemy;
    ...
    useNormalAttackAnimation(EquipmentType_ID, EquipmentCrystal_ID) {
        ...
        if (Normal) var ClipName = Normal.EffectEnemy;
        if (ClipName) this.showAnimationlayer.playAdditive(ClipName);
    ...
    onFinished() {
        console.log('Attack Enemy finish');
